...
export const Button = ({ variant = 'primary', size = 'xl', className, style, label, classNameLabel, styleLabel, children, ...props }: ButtonProps) => {
    return (
        <>
            {label && (
                <label className={composeClasses(classNameLabel)} style={styleLabel || {}}>
                    {label}
                </label>
            )}
            <button className={composeClasses(colorVariants[variant], sizeVariants[size], className)} style={style} {...props}>
                {children}
            </button>
        </>
    )
